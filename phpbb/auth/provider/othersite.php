...
/**
 *
 * This file is written by Igor Peshkov for other site user authentication.
 *
...
namespace dark_diesel\AuthProviderOtherSite\phpbb\auth\provider;

/**
 * Database authentication provider for phpBB3
 * This is for authentication via the integrated other site
 */
class othersite extends \phpbb\auth\provider\base
{
    /**
    ...
    public function login($username, $password)
    {
        ...
        // get other site user
        $sql = 'SELECT *
			FROM ' . $this->config['auth_othersite_user_table'] . "
			WHERE " . $this->config['auth_othersite_user_name_fld']. " = '" . $this->db->sql_escape($username_clean) . "'";
        $result = $this->db->sql_query($sql);
        $other_site_user = $this->db->sql_fetchrow($result);
        ...
        if (!$other_site_user)
        {
        ...
        } elseif ($other_site_user && !$forum_user){
            if ($this->config['ip_login_limit_max'] && $attempts >= $this->config['ip_login_limit_max'])
            {
                return array(
                    'status'		=> LOGIN_ERROR_ATTEMPTS,
            ...
            if ($other_site_user[$this->config['auth_othersite_user_password_fld']] == sha1($password)) {
                return array(
                    'status' => LOGIN_SUCCESS_CREATE_PROFILE,
                    'error_msg' => false,
                    'user_row' => array(
                        "username" => $other_site_user[$this->config['auth_othersite_user_name_fld']],
                        "user_password" => $this->passwords_manager->hash($password),
                        "user_email" => $other_site_user[$this->config['auth_othersite_user_email_fld']],
    ...
    public function acp()
    {
        ...
        return array(
            'auth_othersite_user_table', 'auth_othersite_user_name_fld', 'auth_othersite_user_password_fld', 'auth_othersite_user_email_fld',
        );
    }
    ...
    {
        return array(
            'TEMPLATE_FILE'	=> './../../ext/dark_diesel/AuthProviderOtherSite/adm/style/auth_provider_othersite.html',
            'TEMPLATE_VARS'	=> array(
                'AUTH_OTHERSITE_USER_TABLE'		=> $new_config['auth_othersite_user_table'],
                'AUTH_OTHERSITE_USER_NAME_FLD'		=> $new_config['auth_othersite_user_name_fld'],
                'AUTH_OTHERSITE_USER_PASSWORD_FLD'		=> $new_config['auth_othersite_user_password_fld'],
                'AUTH_OTHERSITE_USER_EMAIL_FLD'		=> $new_config['auth_othersite_user_email_fld'],
            ),
        );
    }
}
